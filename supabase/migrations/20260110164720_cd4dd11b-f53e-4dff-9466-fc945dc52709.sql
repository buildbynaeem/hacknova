-- Drop and recreate the view with SECURITY INVOKER to respect the calling user's permissions
DROP VIEW IF EXISTS public.shipments_secure;

CREATE VIEW public.shipments_secure
...
COMMENT ON VIEW public.shipments_secure IS 'Secure view with SECURITY INVOKER that masks phone numbers based on user role and shipment status. Drivers can only see pickup phone during PICKUP_READY+ and receiver phone during IN_TRANSIT+.';